...
#define MAX_BUF 200 // Maximum size of each history entry

char history[HISTLEN][MAX_BUF];
int oldest = 0;   // Index of the oldest entry in history
int newest = 0;   // Index where the next new entry will go
int num_entries = 0;  // Number of valid entries in history
...
void addToHistory(char* command) {
    strncpy(history[newest], command, MAX_BUF);
    newest = (newest + 1) % HISTLEN;
    if (num_entries < HISTLEN) {
        num_entries++;
    } else {
...
char* retrieveFromHistory(int index) {
...
// "!!" command
if (strcmp(parsedLine->arguments[0], "!!") == 0) {
    char* last_command = retrieveFromHistory(num_entries);
    if (last_command != NULL) {
        strcpy(input, last_command);
        printf("%s\n", input);  // Optional: Print the command being executed
        parsedLine = parseCmdLines(input);
    } else {
        freeCmdLines(parsedLine);
        continue;  // No valid entry found, skip execution
    }
}

// "!n" command
if (parsedLine->arguments[0][0] == '!' && isdigit(parsedLine->arguments[0][1])) {
    int index = atoi(parsedLine->arguments[0] + 1);
    char* history_command = retrieveFromHistory(index);
